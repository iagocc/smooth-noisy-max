...
from mechanims import em_pmf, ld_pmf, rnm_pmf, expected_error_val
...
if __name__ == "__main__":
    ...
    u = utility(x, p)
    epss = [0.1, 0.1269]  # np.logspace(-1, 2, num=30)
    ...
    for eps in epss:
        ...
        err_em = expected_error_val(x, m, pmf_em)
        err_pf = expected_error_val(x, m, pmf_pf)
        err_ld = expected_error_val(x, m, pmf_ld)
        err_rlnm = expected_error_val(x, m, pmf_rlnm)
